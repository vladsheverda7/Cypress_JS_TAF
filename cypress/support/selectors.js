...
const inventoryPage = {
  ...
  getNameList() {
    return cy.get(this.inventoryItem.name).then(($els) => {
      if (!$els.length) return [];
      return [...$els].map((el) => el.innerText.trim());
    });
  },
  getDescriptionList() {
    return cy.get(this.inventoryItem.description).then(($els) => {
      if (!$els.length) return [];
      return [...$els].map((el) => el.innerText.trim());
    });
  },
  getPriceList() {
    return cy.get(this.inventoryItem.price).then(($els) => {
      if (!$els.length) return [];
      return [...$els].map((el) => el.innerText.slice(1).trim());
    });
  },
};
